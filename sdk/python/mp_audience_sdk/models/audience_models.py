...
#   timestamp: 2026-03-09T16:26:39+00:00
...
class Relative(BaseModel):
    ...
    offset: float
    relative_to: Optional[AliasPath] = None
    unit: DateUnit
...
class SequenceOperator(RootModel[Literal["then"]]):
    root: Literal["then"] = Field(
        ...,
        description='Represents operators for expressing ordered event sequences. Examples:\n- "then": user performed Event A, then Event B   { operator: "then", sequence: [{ alias: "a", operand: {...} }, { alias: "b", operand: {...} }] }',
    )
...
class Expression1(BaseModel):
    ...
    sequence: List[SequenceOperand]


class Expression(
    RootModel[Union[UnaryExpression, BinaryExpression, LogicalExpression, Expression1]]
):
    root: Union[UnaryExpression, BinaryExpression, LogicalExpression, Expression1] = (
        Field(
            ...,
...
class SequenceOperand(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    alias: str
